...
public class HttpContentDecoderTest {
    ...
    private static final String JQUERY_STRING;
    private static final byte[] JQUERY_BZ_BYTES;

    static {
        InputStream uncompressed = HttpContentDecoderTest.class.getClassLoader()
          .getResourceAsStream("jquery-3.5.1.min.js");
        try {
            JQUERY_STRING = IOUtils.toString(uncompressed, CharsetUtil.UTF_8);
        } catch (Throwable e) {
        ...
        InputStream compressed = HttpContentDecoderTest.class.getClassLoader()
          .getResourceAsStream("jquery-3.5.1.min.js.br");
        try {
            JQUERY_BZ_BYTES = IOUtils.toByteArray(compressed);
        } catch (Throwable e) {
            throw new ExceptionInInitializerError(e);
        } finally {
    ...
    public void testResponseBrotliDecompression() {
        ...
        String headers = "HTTP/1.1 200 OK\r\n" +
          "Content-Length: " + JQUERY_BZ_BYTES.length + "\r\n" +
          "Content-Encoding: br\r\n" +
          "\r\n";
        ByteBuf buf = Unpooled.wrappedBuffer(headers.getBytes(CharsetUtil.US_ASCII), JQUERY_BZ_BYTES);
        assertTrue(channel.writeInbound(buf));

        Object o = channel.readInbound();
        assertThat(o, is(instanceOf(FullHttpResponse.class)));
        FullHttpResponse resp = (FullHttpResponse) o;
        assertEquals(JQUERY_STRING.length(), resp.headers().getInt(HttpHeaderNames.CONTENT_LENGTH).intValue());
        assertEquals(JQUERY_STRING, resp.content().toString(CharsetUtil.UTF_8));
        resp.release();
    ...
    public void testResponseChunksBrotliDecompression() {
        ...
        int offset = 0;
        while (offset < JQUERY_BZ_BYTES.length) {
            int len = Math.min(1500, JQUERY_BZ_BYTES.length - offset);
            boolean available = channel.writeInbound(Unpooled.wrappedBuffer(JQUERY_BZ_BYTES, offset, len));
            offset += 1500;
            if (offset < JQUERY_BZ_BYTES.length) {
